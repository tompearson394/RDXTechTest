    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            WithViewStore(store) { viewStore in
                Toggle(isOn: viewStore.binding(\.$hasAcceptedTermsOfService)) {
                    Text("I Accept")
                        .frame(maxWidth: .infinity, alignment: .trailing)
                }
                .padding()
            
                NextButton(disabled: !viewStore.hasAcceptedTermsOfService) {
                    viewStore.send(.coordinator(.handleNextPress(fromScreen: .termsOfService)))
                }
            }
        }
